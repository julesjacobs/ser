...
%inspiring extensive research on model-checking and complexity analyses~\cite{BoEnGuHa17,ZeBiBoEnEr19,LaBo20}.
%
The \emph{membership problem}, i.e., the problem of deciding whether a specific interleaving is serializable, was proven to be \texttt{NP}-complete~\cite{Pa79}. This result was later extended to linearizability~\cite{GiKo97} as well as to other consistency models~\cite{BiEn19}. 
...
The \emph{correctness problem}, i.e., the problem of deciding whether \emph{all} executions of a system satisfy this criterion, was proven to be in \texttt{EXPSPACE} if threads are bounded~\cite{AlMcPe96}. Otherwise, for unbounded threads this problem is undecidable~\cite{BoEmEnHa13}, though decidable if the bounded-barrier property holds (which includes our setting, implying that serializability is decidable). Bouajjani et al.~\cite{BoEmEnHa18} further prove that, for specific ADTs, unbounded-thread linearizability can be reduced to VASS coverability which is in \texttt{EXPSPACE}~\cite{Ra78}. 
%
To our knowledge, the \toolname{} toolchain is the first to implement the serializability algorithm by Bouajjani et al. ~\cite{BoEmEnHa13}, while also adapting it to distributed transactions, as well as extending it with a proof certification engine, and adding multiple optimizations to scale to various models of real-world programs. 
...
Other model checkers, e.g., \texttt{SPIN}/\texttt{PARGLIDER}~\cite{Fl04,VeYaYo09,Ho97,VeYa08}, rely on explicit linearization points, which can be quite difficult to determine~\cite{VeYaYo09}.  
...
Static methods have been used to prove linearizability for both bounded~\cite{AmRiReSaYa07,MaLeSaRaBe08} and unbounded systems~\cite{BeLeMaRaSa08,Va09,Va10}. However, these techniques typically build on heuristics or annotated linearization points (for example,~\cite{DrPe14}).  
