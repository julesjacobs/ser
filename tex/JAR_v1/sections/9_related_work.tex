...
Serializability (a.k.a. \emph{atomicity}) was
first defined by Eswaran et al.~\cite{EsGrKoTr76},
%, which can be viewed as a relaxtion of \emph{conflict serializability} 
%and \emph{Strict serializability} (SSR)~\cite{Pa79}. 
and later motivated Herlihy and Wing's~\cite{HeWe87,HeWi90} similar notion of the \emph{linearizability} property for concurrent data structures. 
%,
%giving rise to additional consistency models~\cite{ZhChWa13, RaMeBrKoSi93, La78,AhNeBuKoHu95}, 
%inspiring extensive research on model-checking and complexity analyses~\cite{BoEnGuHa17,ZeBiBoEnEr19,LaBo20}.
%
The \emph{membership problem}, i.e., the problem of deciding if a specific interleaving is serializable, was proven to be \texttt{NP}-complete~\cite{Pa79}. This result was later extended to linearizability~\cite{GiKo97} as well as to other consistency models~\cite{BiEn19}. 
...
The \emph{correctness problem}, i.e., the problem of deciding whether \emph{all} executions of a system satisfy this criterion, was proven to be in \texttt{EXPSPACE} if threads are bounded~\cite{AlMcPe96}. Otherwise, for unbounded threads this problem is undecidable~\cite{BoEmEnHa13}, though decidable if the unbounded threads have the bounded-barrier property (which includes our setting, in which serializability is decidable). Bouajjani et al.~\cite{BoEmEnHa18} further prove that, for specific ADTs, unbounded-thread linearizability can be reduced to VASS coverability which is in \texttt{EXPSPACE}~\cite{Ra78}. 
%
To our knowledge, the \toolname{} toolchain is the first to implement the serializability algorithm by Bouajjani et al. ~\cite{BoEmEnHa13}, while also adapting it to to distributed transactions, as well as extending it with a proof certification engine, and adding multiple optimizations to scale it to various models of real-world programs. 
...
%\medskip
%\noindent
\subsection{Model Checking and Runtime Verification}
%
Wang and Stoller~\cite{WaSt06a,WaSt06b} proposed runtime checks for serializability as well as for conflict/view‐serializability.  
Lamport's TLA logic~\cite{La94} is capable of expressing various forms of serializability~\cite{CoOlPnTuZu07}, however, such approaches~\cite{SoVaVi20,Ho24} are limited to bounded systems due to finite‐state tools, such as \texttt{TLC} and \texttt{Apalache}~\cite{YuMaLa99,KoKuTr19}.  
Model checkers that rely on heuristics or Enumeration include \texttt{LinTSO}~\cite{BuGoMuYa12}, \texttt{Line-up}~\cite{BuDeMuTa10} (built on \texttt{CHESS}~\cite{MuQaBaBaNaNe08}), \texttt{Violat}~\cite{EmEn19} and its precursor~\cite{EmEn18}, PAT‐based refinement checking~\cite{LiChLiSuZhDo12,SuLuDoPa09,LiChLiSu09,Zh11}, and bridge‐predicate methods~\cite{BuNeSe11,BuSe09}. 
Recently, \texttt{RELINCHE} has been proposed for bounded linearizability~\cite{GoKoVa25}. Other tools for this task include \texttt{Lincheck}~\cite{KoDeSoTsAl23}, \texttt{CDSSpec}~\cite{OuDe17} (for \texttt{C/C++11}), and \texttt{SAT}‐based~\cite{BiHeCa09,ViWeMa15} methods~\cite{BuAlMa07}.  
Furthermore, symbolic testing techniques~\cite{EmEnHa15} can be used to expose violations of observational refinement~\cite{FiOhRiYa10,BoEmCoHa15}.  
Other model checkers, e.g. \texttt{SPIN}/\texttt{PARGLIDER}~\cite{Fl04,VeYaYo09,Ho97,VeYa08} and others, rely on explicit linearization points, which can be quite difficult to determine~\cite{VeYaYo09}.  
Overall, the existing methods are typically bounded, incomplete, or assume some prior knowledge on the concurrent system in question.  
By contrast, our approach handles unbounded threads and as far as we are aware, is the only one capable of producing a serializability certificate proof.
...
Static methods have been used to prove linearizability for both bounded~\cite{AmRiReSaYa07,MaLeSaRaBe08} and unbounded systems~\cite{BeLeMaRaSa08,Va09,Va10}. However, these techniques typically build on on heuristics or annotated linearization points (for example,~\cite{DrPe14}).  
	Lian and Feng~\cite{LiFe13} have also put forth a logic for non‐fixed points.  
%	Other approaches also depend on linearization points~\cite{OhRiVeYaYo10,ZhPeHa15,AbJoTr16}.  
	However, we note that annotation‐based analyses~\cite{OhRiVeYaYo10,ZhPeHa15,AbJoTr16} can be inconclusive, as failures can also stem from incorrect annotation by the user, rather than from ``true'' violations of the property checked~\cite{BoEmCoHa15}.

%\medskip
%\noindent
\subsection{Manual Proofs and Additional Approaches}
Tasiran~\cite{Ta08} proved serializability for the \texttt{Bartok-STM}, while Colvin et al.~\cite{CoGrLuMo06} used input/output automata for list‐set linearizability.  
Various simplifications exist for specific data structures~\cite{BoEmEnMu17,FeEnMoRiSh18}.
%	, and verification has been pursued with~\cite{CoGrLuMo06} or without~\cite{DoGrLuMo04} proof assistants.  
	Additional linearizability results include Chakraborty et al.~\cite{ChHeSeVa15} on queues, Wing and Gong's~\cite{WiGo93} on unbounded FIFO/priority queues, and Cerný et al.’s \texttt{CoLT} for linked heaps (which, only under bounded threads~\cite{CeRaZuChAl10}, is complete).  
	Bouajjani et al.~\cite{BoEnWa17} introduced a recursive priority‐queue detector for violations, that is akin to their stack/queue techniques~\cite{BoEmEnHa18}.
%
%\medskip
%\noindent
%\textbf{Further approaches.}
Additional approaches include theorem proving~\cite{CoDoGr05,DeScWe11}, testing~\cite{WiGo93,PrGr12,PrGr13,EmEn17,Lo17}, and the use of additional verification frameworks~\cite{BoEmEnMu17,FeEnMoRiSh18,EnKo24}.
