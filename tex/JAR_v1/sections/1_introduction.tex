...
%
Relevant background on Petri nets, their rechability problem and semilinear sets is covered in \Cref{sec:background}.
%
Our \toolname{} language and network system abstraction are both presented in \Cref{sec:problem-definition}.
%
Our decision procedure, as well as its proof certificate engine are presented in \Cref{sec:formal-results}.
%
Our various optimizations, the bidirectional slicing soundness proof, and their evaluation is covered in \Cref{sec:optimizations}.
%
...
%
We also include an appendix with technical details. Furthermore, our tool, benchmarks, and raw experimental results are all publicly available online, and hosted on a permanent repository~\cite{ArtifactRepository}.
% 



\medskip
\noindent{\bf Note.} This is an extended version of our paper, titled ``\emph{Deciding Generalization in Deep Learning}''~\cite{AmBaAmJa26}, which appeared at the Tools and Algorithms for the Construction and Analysis of Systems (TACAS) 2026 conference. 
In the original paper, we presented a brief
description of our method and the evaluation. In this extended version, we significantly enhance our
original camera-ready subimssion across multiple axes, as elaborated next. 
\begin{enumerate}
	\item 
	We add a full section with various \toolname{} programs, with both serializable and non-serializable cases. These also include programs we evaluated and that are motivated by real world SDN protocols, such as stateful firewalls, network monitors, and more.
	
	\item 
	We include the semantics of our \toolname{} programs, formalized in full.
	
	\item 
	We extend the original subsection on optimizations to a full section, in which we present a proof for the soundness of our bidirectional slicing optimization. We also include extensive evaluation of these optimizations, demonstrating that these reduce the space and time complexity by orders of magnitude. These experiments were not covered in the main text of our TACAS proceedings paper.
	
	\item 
	We include a formal translation of network systems to Petri nets.
	
	\item 
	We add examples for both proofs of serializability and counterexamples of non-serializable programs.
	
	\item 
	We also include extended background material, and additional appendices.

\end{enumerate}

We believe the additional material constitutes well over 30\% increase in volume compared to our original paper, which included 17 pages of text (excluding references).
