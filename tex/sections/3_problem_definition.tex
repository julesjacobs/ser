...
// end
	  ...
	  \draw[->] (state1) -- node[above] {\scriptsize $\texttt{X=0} \to \texttt{X=1}$} node[below] {\scriptsize $\texttt{X=1} \to \texttt{X=1}$} (state2);
	  
	  % From state2 to final states
	  \draw[->] ([yshift=4pt]state2.east) to[out=50,in=180] node[above, sloped] {\scriptsize $\texttt{X=0} \to \texttt{X=0}$} (state3.west);
	  \draw[->] ([yshift=-16pt]state2.east) to[out=-50,in=180] node[below, sloped] {\scriptsize $\texttt{X=1} \to \texttt{X=0}$} (state4.west);
