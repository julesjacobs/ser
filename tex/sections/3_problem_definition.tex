...
\todo{add other rebuttal promises}
\todo{check semantics}
...
%
Values are naturals; booleans use $0/1$. Locals $\ell:\mathrm{Locals}\to\mathbb{N}$, globals $g:\mathrm{Globals}\to\mathbb{N}$.
Expression configs $\cfg{e}{\ell}{g}$, program configs $\langle g \mid (e,\ell)\cdot\Pi\rangle$ (FIFO queue).
...
\begin{mathpar}
	...
	\inferrule*[right=L-READ]{\ell(x)=v}{\cfg{x}{\ell}{g} \step \cfg{v}{\ell}{g}}
	\and
	\inferrule*[right=G-READ]{g(X)=v}{\cfg{X}{\ell}{g} \step \cfg{v}{\ell}{g}}
	
	\inferrule*[right=ASSIGN-STEP]{\cfg{e}{\ell}{g} \step \cfg{e'}{\ell'}{g'}}{\cfg{x := e}{\ell}{g} \step \cfg{x := e'}{\ell'}{g'}}
	\and
	\inferrule*[right=ASSIGN-DONE]{ }{\cfg{x := v}{\ell}{g} \step \cfg{v}{\update{\ell}{x}{v}}{g}}
	
	\inferrule*[right=GASSIGN-STEP]{\cfg{e}{\ell}{g} \step \cfg{e'}{\ell'}{g'}}{\cfg{X := e}{\ell}{g} \step \cfg{X := e'}{\ell'}{g'}}
	\and
	\inferrule*[right=GASSIGN-DONE]{ }{\cfg{X := v}{\ell}{g} \step \cfg{v}{\ell}{\update{g}{X}{v}}}
...
\begin{mathpar}
	\inferrule*[right=S-STEP]{\cfg{e}{\ell}{g} \step \cfg{e'}{\ell'}{g'}}{\langle g \mid (e,\ell)\cdot \Pi\rangle \pstep \langle g' \mid (e',\ell')\cdot \Pi\rangle}
	\and
	\inferrule*[right=S-YIELD]{ }{\langle g \mid (\yieldkw ; e,\ell)\cdot \Pi\rangle \pstep \langle g \mid \Pi \cdot (e,\ell)\rangle}
	\and
	\inferrule*[right=S-RETIRE]{v\in\mathbb{N}}{\langle g \mid (v,\ell)\cdot \Pi\rangle \pstep \langle g \mid \Pi\rangle}
	\and
	\inferrule*[right=S-REQUEST]{i\in\{1,\dots,n\}}{\langle g \mid \Pi\rangle \pstep \langle g \mid \Pi \cdot (e_i,\ell_0)\rangle}
\end{mathpar}
...
A transition \(\longrightarrow\) modifies the triple \((g,\mathcal{P}, M)\) by either
...
the result set \(M\) is the final multiset of request/response pairs that arose during the run.
...
{(g,\mathcal{P},M) \rightarrow (g,\; \mathcal{P}\uplus\{(\ell,{\color{ForestGreen}\blacklozenge_\text{r}})\},\; M)}
\]
\[
\text{(Processing Step)}\quad
\infer{((g,\ell),(g',\ell'))\in\delta}
{(g,\; \mathcal{P}\uplus\{(\ell,{\color{ForestGreen}\blacklozenge_\text{r}})\},\; M)
	\rightarrow
	(g',\; \mathcal{P}\uplus\{(\ell',{\color{ForestGreen}\blacklozenge_\text{r}})\},\; M)}
\]
\[
...
{(g,\; \mathcal{P}\uplus\{(\ell,{\color{ForestGreen}\blacklozenge_\text{r}})\},\; M)
	\rightarrow
	(g,\; \mathcal{P},\; M \uplus \{({\color{ForestGreen}\blacklozenge_\text{r}},{\color{red}\blacklozenge_\text{s}})\})}
\]
...
\[
(g_0,\varnothing,\varnothing) \rightarrow (g_1,\mathcal{P}_1,M_1)
\rightarrow \cdots \rightarrow (g_n,\mathcal{P}_{n-1},M_{n-1}) \rightarrow (g_n,\varnothing,M_n).
\]

\noindent
A full run is also denoted \((g_0,\varnothing,\varnothing)\rightarrow^{*}(g_n,\varnothing,M)\), and is called an \textit{interleaved} run if the $\mathcal{P}_i$ may contain multiple requests and $\mathcal{P}_n=\varnothing$.
%
...
= \bigl\{\, M \in \mathrm{Multiset}(\mathit{REQ}\times \mathit{RESP})
\;\big|\; \exists\ \text{interleaved run } (g_0,\varnothing,\varnothing)\rightarrow^{*}(g_n,\varnothing,M) \,\bigr\}
\]
\[
\mathrm{Ser}(\mathcal{N})
= \bigl\{\, M \in \mathrm{Multiset}(\mathit{REQ}\times \mathit{RESP})
\;\big|\; \exists\ \text{serial run } (g_0,\varnothing,\varnothing)\rightarrow^{*}(g_n,\varnothing,M) \,\bigr\}.
\]
