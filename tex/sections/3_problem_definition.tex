...
%
% depicts the program in Fig.~\ref{fig:code2ExampleNS}.
% as follows:
We note that we depicted only part of the states that are \textit{reachable}, in our finite-state setting.
%
%Next, we present in Fig.~\ref{fig:code2ExampleNS} , which serves as a mapping from requests ({\color{ForestGreen}$\blacklozenge_\text{main}$}) to responses ({\color{red}$\blacklozenge_0$}, {\color{red}$\blacklozenge_1$}).
...
%		\vspace{0pt}% align this minipage at the top
%		\centering
%		\includegraphics[width=\linewidth]{plots/code_2_NS.png}
%		\caption*{(a) Network system for the program in Listing~\ref{lst:MotivatingExample2NonSer}.}
%	\end{minipage}
%	\hfill
%	% right: the three definitions
...
%		\underbrace{
%			\tikz[baseline=(n.base)]
%			\node[diamond,aspect=2,fill=ForestGreen!20,inner sep=4pt] (n) {main};
%		}_{\mathrlap{\text{“main” request}}}
%		\;\rightarrow\;
%		\bigl(
%		\tikz[baseline=(y.base)]
%		\node[rectangle,fill=brightyellow,inner sep=2pt,rounded corners=1pt] (y) {\large y=0};
%		,\;
...
%		\bigl[
%		\bigl(\,
%		\tikz[baseline=(y2.base)]
%		\node[rectangle,fill=brightyellow,inner sep=2pt,rounded corners=1pt] (y2) {\large y=0};
%		,\;
...
%		\;\rightarrow\;
%		\tikz[baseline=(d.base)]
%		\node[diamond,aspect=2,fill=RedViolet!20,inner sep=4pt] (d) {0};
%		\bigr]
%		,\;\ldots
%		\Bigr\}
%		\]
...
%		\bigl[
%		\bigl(
%		\tikz[baseline=(x0.base)]
%		\node[rectangle,fill=lightchacki,inner sep=2pt,rounded corners=1pt] (x0) {\large X=0};
%		,\;
%		\tikz[baseline=(y3.base)]
%		\node[rectangle,fill=brightyellow,inner sep=2pt,rounded corners=1pt] (y3) {\large y=0}
%		\bigr)
%		\;\rightarrow\;
%		\bigl(
%		\tikz[baseline=(x1.base)]
%		\node[rectangle,fill=lightchacki,inner sep=2pt,rounded corners=1pt] (x1) {\large X=1};
%		,\;
%		\tikz[baseline=(y4.base)]
%		\node[rectangle,fill=brightyellow,inner sep=2pt,rounded corners=1pt] (y4) {\large y=0}
%		\bigr)
%		\bigr]
%		,\;\ldots
%		\Bigr\}
...
%	
%	\caption{%
%		(a) The network system. \quad
%		(b) The three components of the transition‐relation encoding:
%		$req$, $resp$, and~$\delta$.}
%	\label{fig:combined-NS-and-defs}
%\end{figure}
