...
%\end{figure}
    %
This NS setting is motivating by software-defined networking systems. Specifically, spawning a request corresponds to sending a packet, with each local variable mapped to a unique packet header fields; global variables correspond to variables on programmable switches, as they are shared among all packets (threads) of the system, visiting the switch. Throughout this section, we will refer to packets/threads/requests interchangeably. 
...
\item $\mathit{resp} \subseteq L \times \mathit{Res}$ is a response transition relation, which describes which thread turn into which responses
