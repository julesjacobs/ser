...
\newcommand{\grammartag}[1]{\emph{#1}}
\begin{figure}[t]
    \begin{align*}
    e ::= &&& \grammartag{Single expression}\\
       | & \quad 0 \mid 1 \mid 2 \mid \ldots                                && \grammartag{Numeric constants} \\
       | & \quad \nondet                                 && \grammartag{Nondeterministic value: 0 or 1}\\
       | & \quad x := e                            && \grammartag{Write to local variable (packet field)} \\
       | & \quad x                                 && \grammartag{Read from local variable (packet field)} \\
       | & \quad X := e                            && \grammartag{Write to global variable (switch variable)} \\
       | & \quad X                                 && \grammartag{Read from global variable (switch variable)} \\
    ...
    p ::=
        & \quad \requestkw\ name_1\ \{\ e_1\ \}&&\grammartag{Whole program:}\\[-0.5em]
        & \quad \qquad \vdots &&\grammartag{set of request handlers}\\
