...
\begin{itemize}
	...
	\item \todo{update benchmark overview}
...
\caption{Average and median certificate generation, certificate validation, and total running times (in ms) for serializable, non‐serializable, and all benchmarks. These values are rounded to the nearest integer, to reduce clutter.}
...
See Fig.~\ref{fig:timeout_cumulative_solved_log}.

\subsubsection{Space Optimization}

\paragraph{Size reduction of Petri Nets.}

bla
See the results in Fig.~\ref{fig:petri_size_reduction}
...
\paragraph{Size reduction of semilinear sets.}
