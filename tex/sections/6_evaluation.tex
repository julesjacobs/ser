...
%
Within the time limit, our tool solved 26 of the 27 serializable benchmarks and 19 of the 20 non-serializable benchmarks (see Table~\ref{tab:benchmarks-all}).
%
...
%
We also observe a clear runtime split based on serializability. For serializable benchmarks, certificate validation takes much longer than proof generation. In non-serializable cases, the opposite holds: counterexample generation dominates the runtime. This makes sense, since generating a counterexample only requires a polynomial-time simulation of the network to confirm its feasibility.
%
