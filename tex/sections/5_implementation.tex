...
\begin{enumerate}
	...
	Our framework receives either a \texttt{SER} program with the syntax described in Sec.~\ref{sec:problem-definition}; or a \texttt{JSON} file directly encoding a networking system. In the case of the former, an additional step takes place, parsing the input (using the \texttt{parser.rs} module) to an expression tree and then translated to an equivalent NS, represented as a struct in the \texttt{ns.rs} module. 
	
	\item \textbf{Petri Net Conversion.} The NS is then translated into a Petri Net (see the \texttt{petri.rs} module) which represents the interleavings as presented, for example, in subsec.~\ref{subsec:ns-not-serializable}.
...
%	This conversion is done by the following pipeline: (1) the NS is translated to an NFA encoding all possible request/response pairs of the input program; (2) This NFA is translated to a regex (via Kleene's Theorem) and then projected (via the Parikh Image) to a semilinear set. The (finite) encoding of the semilinear set symbolically represents all outputs attained via serializable executions of the program, running for an unbounded number of steps; (3) finally, we complement the semilinear set to encode all outputs unattainable via serial executions. 
	...
	%
	In order to expedite the search, a ``large'' (PN, query) pair are replaced with multiple pruned PNs, each coupled with a sub-query encoding a separate disjunct. 
...
are recorded. 
%The logger outputs 
Structured \texttt{CSV} and \texttt{JSON} logs are generated along with
% analysis. 
	%
%	We also assemble 
an \texttt{HTML} report embedding the original net, symbolic constraints, reachability results, proof outlines, and profiling graphs.
