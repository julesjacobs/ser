...
\begin{enumerate}
	\item \textbf{Input \& Parsing.} Our model receives one of two textual representation of the program in question: (i) a \textit{ser} program with the syntax described in Sec.\todo{add}; or (ii) a \textit{json} file which directly encode the transitions of our networking system. Given that the user feeds the former, an additional parsing step takes place, in which the input program is parsed to an expression tree and then translated to an equivalent networking system.
