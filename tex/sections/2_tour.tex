...
%
Note that the presence of the \textit{else} branch renders the program serializable, as intuitively, for every value of x that can change due to an interleaving and updates via the ``if'' branch, there is a serial execution in which an ``else'' branch is chosen and thus this program is serializable.
%
...
%
Specifically, given at least $i=5$ interleavings of in-flight {\color{ForestGreen}$\blacklozenge_\text{flip}$} requests, it is possible for a {\color{ForestGreen}$\blacklozenge_\text{main}$} request to terminate and bypass all while loops, something that cannot occur in serializable executions.
%
