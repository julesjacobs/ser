...
\(M_0\in\mathbb N^P\) the initial marking (initial distribution of tokens across the places). Note that throughout this work, we use the terms \textit{state} and \textit{marking} interchangeably.
...
\]
which represents the consumption of inputs tokens (from the input places), and the production of new ones (to the output places). In this case, we denote $M \;\xrightarrow{t}\; M'$ that $M'$ is reachable from $M$.
%
By extension, we say that a marking \(M'\) is \emph{reachable} from a marking $M$ by firing a sequence $\sigma = t_1 .. t_k \in T^*$ if there exists some markings $M_0, .., M_k$ such that $M= M_0$, $M'= M_k$ and $M_i \; \xrightarrow{t_{i+1}} M_{i+1}$ for all $i$ in  $0..(k-1)$.
%%
Finally, $R(N)$ denotes the set of the reachable markings:
\[
...
Ackermann-complete~\cite{CzWo22,leroux_reachability_2022} complexity.
%
An example for a toy Petri Net, and both reachable and unreachable markings, appears in Appendix~\ref{appendix:toyPN}.
%
In our work, we focus on the verification of \textit{reachability properties}, meaning properties on the states that a net $N$ can reach. We want to check if it is possible to reach a marking that satisfies a formula, say $F$, that is a combination of linear constraints between places. Such property is said \textit{reachable} (or \sat by analogy with SAT solving) if there exists some $M$ in $R(N)$ such that $M \models F$, and \textit{unreachable} otherwise (\unsat). 
...
%
More interestingly, if $F$ is not reachable, the theory of Petri nets guarantees the existence of an inductive certificate $C$, expressed as a Presburger formula, that implies $F$~\cite{leroux_general_2009,leroux_general_2010}. Such certificate satisfies the following constraints:
(i) $M_0 \models F$;
...
Regarding the aforementioned program, we automatically generate the following reachability query for the Petri net of Figure~\ref{fig:code1ExamplePN} (which, if reachable, indicates the program is not serializable):
...
\]
This asserts the existence of a marking with no tokens on $P_1,P_4,P_5,P_6$,$\textcolor{red}{P_7}$, at least one token on $\textcolor{red}{P_8}$, and arbitrary tokens on $\textcolor{blue}{P_2},\textcolor{blue}{P_3}$.  In fact, the target marking is reachable (see Table~\ref{tab:PetriNetFiringCounterexample}):
\[
...
%
Non-serializability corresponds to the Petri Net (in Fig.~\ref{fig:code3ExamplePN}) being able to reach a marking satisfying the following formula:
