...
\]
which represents the consumption of input tokens (from the input places), and the production of new ones (to the output places). In this case, we denote by $M \xrightarrow{t} M'$ that $M'$ is reachable from $M$.
%
By extension, we say that a marking \(M'\) is \emph{reachable} from a marking $M$ by firing a sequence $\sigma = t_1 .. t_k \in T^*$ if there exists some sequence of markings $M_0, .., M_k$ such that $M= M_0$, $M'= M_k$ and $M_i \; \xrightarrow{t_{i+1}} M_{i+1}$ for all $i$ in $0..(k-1)$, and we denote this as $M \; \xrightarrow{\sigma} M'$.
%%
...
%
More interestingly, if $F$ is not reachable, the theory of Petri nets guarantees the existence of an inductive certificate $C$, expressed as a Presburger formula, that implies $F$~\cite{Le09}. Such a certificate satisfies the following constraints:
(i) $M_0 \models F$;
...
%
Furthermore, unlike the previous example, this semilinear target set of markings (encoding request/responses of non-serial executions) is \textit{unreachable}, as witnessed by the following inductive invariant.
