...
\(M_0\in\mathbb N^P\) the initial marking (initial distribution of tokens across the places).  
...
More interestingly, if $F$ is not reachable, the theory of Petri nets guarantees the existence of an inductive certificate $C$--a Presburger formula satisfying:
\begin{enumerate}
	\item $M_0 \models F$
	\item For all transitions $M \xrightarrow{t} M'$, if $M \models C$4 then $M' \models C$,
\item $C \Rightarrow \neg F$, i.e., all markings satisfying $C$ falsify $F$.
\end{enumerate} 
Such certificates act as formal proofs of unreachability~\cite{leroux_general_2009,leroux_general_2010}.
