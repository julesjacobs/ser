...
%	\textit{Notation.}
	...
	\begin{aligned}
		...
		\mid {\color{ForestGreen}\blacklozenge_{\mathit{req}}}\in\mathit{REQ},\,\ell\in EXP\times L\bigr\},\\[1ex]
		P_{REQ,RESP}
...
%	\]
	...
	with \(G\) being the global states, \(L\) being the in‐flight local states, \(EXP\) being the snippets of the remaining program (for simplicity, we denote \(l\in EXP\times L\)); and \(\mathit{RESP}\) denoting the response labels.
...
The interleaving Petri net (step 2) has $O(|G| + (|L|\times |EXP|) + (|\mathit{REQ}| \times |\mathit{RESP}|))$ places and $O(|\mathit{REQ}| + |\delta| + |\mathit{RESP}|)$ transitions.
