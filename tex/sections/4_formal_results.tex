...
%
By extension, we say that a marking \(M'\) is \emph{reachable} from a marking $M$ by firing a sequence $\sigma = t_1 .. t_k \in T^*$ if there exists some markings $M_0, .., M_k$ such that $M= M_0$, $M'= M_k$ and $M_i \; \xrightarrow{t_{i+1}} M_{i+1}$ for all $i$ in the range $0..(k-1)$.
%%
Finally, we denote $R(N)$ the set of the reachable markings defined as
\[
...
If a property $F$ is reachable in $N$, then a witness sequence $\sigma \in T^*$ and marking $M$ such that: 
\[
