...
Next, we give an example motivating the on the use of an automatic checker for program serializability.In particular, we will show how even small, toy programs can be surprisingly difficult to reason about when assessing their serializable behavior.
%
Observe the simple program depicted in Listing~\ref{lst:MotivatingExample1Ser}. Each request call \textit{foo} gives rise to a fresh in-flight	packet. The variable ``X'' is global, and shared among all in-flight requests, while the variable ``y'' is local variable, per each request. 
%
As there are no yields, it is straightforward to see that the program is trivially serializable, with every request \textit{foo}: (i) assigning 1 to the global variable X; (ii) assigning y to have the value of X, i.e. 1; (iii) assigning X the value 0; and (iv) returning y. This results to every (request,response) pair to be of the form \textit{(foo, 1)}.
%
...
By adding a global lock variable, even if an interleaving occurs, the lock will allow only the first request to terminate, and hence release the lock for the next request. Hence, despite having yields, we attain serializable executions, and only \textit{(foo,1)} request/response pairs are produced, similar to the original program in Listing~\ref{lst:MotivatingExample1Ser}.
