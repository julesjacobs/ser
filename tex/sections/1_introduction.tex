...
\begin{lstlisting}[caption={Snapshot-based monitor deactivation (not serializable, as it can return 0 active monitors)}]
    request init:
        if (X == 0):
            N_1_ACTIVE := 1
            N_2_ACTIVE := 1
    ...
    request main:
        ...
        if (n_1_active_snapshot == 1): 
            if (n_2_active_snapshot == 1):
                // both nodes active 
                if (?): // choose whether to deactivate a monitor
                    if (?): // choose which monitor to deactivate
                        N_1_ACTIVE := 0
                    else:
                        N_2_ACTIVE := 0

        // total active nodes
        return N_1_ACTIVE + N_2_ACTIVE 
\end{lstlisting}
\end{minipage}
