...
\todo{5 papers (OOPSLA 2024 + another 4) from slack DM channel with Mark}
...
Serializability was first formalized by Eswaran et al.~\cite{EsGrKoTr76} as a 
...
specific interleaving is serializable. This has been proven to be NP-complete 
by Papadimitriou~\cite{Pa79}, a result that was later extended~\cite{BiEn19} to 
other consistency models.
Regarding linearizability, the easier, membership problem is NP-complete in 
...
EXPSPACE~\cite{AlMcPe96} when bounding the number of threads, linearizability 
...
%
\todo{check if the next two lines correspond to static analysis}
...
%
\todo{for strict serializability}
...
correctness problem for conflict serializability is decidable (and in PSPACE) 
...
EXPTIME-complete). Their key insight reveals that while the conflict graph 
...
Various work focus on checking conflict serializability,.e.g., Farzan and 
...
%
\todo{add parts on invariant inference}
