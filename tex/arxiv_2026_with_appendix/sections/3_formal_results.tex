...
\begin{tcolorbox}[colback=black!5!white, colframe=black, boxrule=1pt]
	...
	which is reachable by the PN in Fig.~\ref{fig:code2ExamplePN}. The full firing sequence leading to marking $M^*$ is given in Table~\ref{tab:PetriNetFiringCounterexample} (in Appendix~\ref{appendix:non-serializable-execution-example}).
	%
...
%
An extensive empirical evaluation of these optimizations appears in subsec.~\ref{subsec:optimization-results} of the appendix.

\medskip
\noindent
\textbf{(1) Bidirectional slicinig.}  
...
The size of the generated semilinear set is not only impacted by how the
	...
	\smallskip
	\noindent
	As we later demonstrate (in~\Cref{sec:evaluation}), our optimizations expedite the search procedure and make the representations \textit{significantly} more compact. This, in turn, enables deciding serializability for instances that are otherwise intractable.
