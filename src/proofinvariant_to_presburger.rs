...
pub fn eliminate_backward(
    ...
    let new_formula = Formula::Or(vec![
        proof_invariant.formula.clone(),
        places_nonzero
    ]);
...
mod tests {
    ...
    fn test_eliminate_forward() {
        ...
        assert_eq!(result.variables, vec!["x".to_string(), "p1".to_string(), "p2".to_string()]);
