...
with \(G\) being the global states, \(L\) being the local states (in the case of a \toolname-derived NS this is the coupling of the local variables and the remaining \toolname{} program to execute), \(\mathit{REQ}\) denotes the request labels; and \(\mathit{RESP}\) denotes the response labels.
...
\begin{alignedat}{3}
	...
	&&\text{for each }((\ell,g),(\ell',g'))\in\delta,\\
...
Where for the last two cases, \({\color{ForestGreen}\blacklozenge_{\mathit{req}}}\) concerns requests that eventually gave rise to a local state \(\ell \in L\) that originated downstream (during execution).
...
Define the projection \(\pi\) to be markings solely on all places representing completed request/response pairs (with the exception of a single token on a state in \(P_G\)).
%\[
