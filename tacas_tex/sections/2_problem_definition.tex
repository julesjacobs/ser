...
%
Specifically, we focus on reachability of a linear-constraint formula $F$; it is \sat\ if some marking $M\in R(N)$ satisfies $F$ (denoted $M \models F$), and otherwise \unsat{} (see the toy example in 	Appendix~\ref{appendix:toyPN}).
%
...
%	whereas \emph{interleaved} runs may have multiple requests in flight at once.
	%
	%
	Let \(\mathrm{Int}(\mathcal{N})\) and \(\mathrm{Ser}(\mathcal{N})\)  respectively denote the (infinite) sets of request/response multisets, for interleaved and serial runs:
%
\[
...
An NS $\mathcal{N}$ is \emph{serializable} if \(\mathrm{Int}(\mathcal{N})=\mathrm{Ser}(\mathcal{N})\), i.e., every multiset of request/response pairs obtained by an interleaved execution can also be obtained serially.
