...
For $\Sigma=\{a_1,\dots,a_k\}$ and $w\in L\subseteq\Sigma^*$, 
$\mathsf{Parikh}(w)=\{\,a_i^{|w|_{a_i}}\mid a_i\in\Sigma\,\}$ denotes the multiset of symbols occurring in $w$.
...
%	&&\text{read from global variable}\\
	...
	&& \text{handlers set; denote } name_i\{e_i\}\in P_0
\end{aligned}
\]
...
\noindent
\textbf{States.}
A (global) \emph{network state} is a triple $(g,\mathcal{R},\mathcal{Z})$ where
$g \in G$ is the current global network state,
$\mathcal{R} \in \mathrm{Multiset}(L \times \mathit{REQ})$ is a multiset of in-flight requests (i.e., local assignments of each thread in the current timestep, and the original request string that spawned it),
...
A transition \(\longrightarrow\) modifies the triple \((g,\mathcal{R}, \mathcal{Z})\) by either
...
the result set \(\mathcal{Z}\) is the final multiset of request/response pairs that arose during the run.
...
{(g,\mathcal{R},\mathcal{Z}) \rightarrow (g,\; \mathcal{R}\uplus\{(\ell,{\color{ForestGreen}\blacklozenge_\text{r}})\},\; \mathcal{Z})}
\]
\[
\text{(Processing Step)}\quad
\infer{((\ell, g),(\ell', g'))\in\delta}
{(g,\; \mathcal{R}\uplus\{(\ell,{\color{ForestGreen}\blacklozenge_\text{r}})\},\; \mathcal{Z})
	\rightarrow
	(g',\; \mathcal{R}\uplus\{(\ell',{\color{ForestGreen}\blacklozenge_\text{r}})\},\; \mathcal{Z})}
\]
\[
...
{(g,\; \mathcal{R}\uplus\{(\ell,{\color{ForestGreen}\blacklozenge_\text{r}})\},\; \mathcal{Z})
	\rightarrow
	(g,\; \mathcal{R},\; \mathcal{Z} \uplus \{({\color{ForestGreen}\blacklozenge_\text{r}},{\color{red}\blacklozenge_\text{s}})\})}
\]
...
(\text{with } \mathcal{R}_{n}=\varnothing,\mathcal{Z}_n=\mathcal{Z})
 %\varnothing,Z
\]
It is said to be \textit{serial} if each $\mathcal{R}_i$ has \textit{at most} one request.
	%
...
\begin{aligned}
	...
	&\qquad\quad\qquad\quad e \text{ is a suffix of } e_i \text{ starting from } \yieldkw \text{ or initialization} \,\bigr\},\\[0.3ex]
