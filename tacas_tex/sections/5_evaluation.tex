...
We ran on a Lenovo ThinkPad P16s (16 AMD cores, 64 GB RAM, Ubuntu 24.04.2).
%
%We plan on making all our code, benchmarks raw results publicly available with the final version of this paper.
%
We use \texttt{SMPT}~\cite{AmDa23} (built on \texttt{Z3}~\cite{DeBj08}) as the PN model checker; exact versions/commits are in the artifact manifest. Code, benchmarks, and full results are public~\cite{ArtifactRepository}.



\subsection{Runtime Results}
We ran \toolname{} on 47 benchmarks (27 serializable, 20 non-serializable), measuring solver time and end-to-end time (including certificate or counterexample validation). Experiments ran on 16 cores with all optimizations and a 500s timeout. Within time, \toolname{} solved 26/27 serializable and 19/20 non-serializable cases (summary in Table~\ref{tab:stats-summary}; full results in Table~\ref{tab:benchmarks-all}).
%
...
%
Median total runtime was $1{,}909$ ms overall; $2{,}238.5$ ms for serializable and $830$ ms for non-serializable. Averages were $32{,}898.38$ ms overall; $25{,}530.69$ ms (serializable) and $42{,}980.47$ ms (non-serializable). Runtime splits by outcome: non-serializable cases spend time in counterexample generation, while serializable cases spend time in certificate validation (counterexample validation is just polynomial-time simulation).
%
...
\begin{center}
	\begin{minipage}[htbp]{0.48\textwidth}
		...
		\captionof{figure}{Solved instances (\texttt{TIMEOUT} $500$ s).}
