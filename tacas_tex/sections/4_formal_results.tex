...
%	
Next, we translate the NS into a Petri Net. The \textit{non-sink places} of the PN represent either (i) global state assignments; and (ii) local states of an in-flight packet. The \textit{sink} places represent request/response pairs of terminating packets.
%
...
%
Formally, let $N=(P,T,\Pre,\Post,M_0)$ be a Petri net, and $S\subseteq\mathbb{N}^P$ a target set. 
%
...
% invoke the solver.
We include in Appendix~\ref{appendix:BidirectionalProof}
an example depicting this in optimization (see Fig.~\ref{fig:bidirectional_pruning}), as well as a proof of Theorem~\ref{thm:bidirectional-pruning}:
...
\begin{theorem}[Bidirectional Pruning Soundness]
	...
	%
	Let $N' = \bigl(P',\,T',\,\Pre|_{P'\times T'},\,\Post|_{P'\times T'}, M_0|_{P'}\bigr)$ be the net after applying the bidirectional pruning algorithm (for some \(P \subseteq P', T \subseteq T'\)).
	%
